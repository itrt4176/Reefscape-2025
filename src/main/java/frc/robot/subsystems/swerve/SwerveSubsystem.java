...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    try {
      ...
      // via JSON files.
      // swerveDrive = new
      // SwerveParser(directory).createSwerveDrive(maximumSpeed,
      ...
      AutoBuilder.configure(this::getPose,
          ...
          (speedsRobotRelative, moduleFeedForwards) -> {
            if (enableFeedforward) {
              swerveDrive.drive(
                  speedsRobotRelative, swerveDrive.kinematics
  ...
  private Command driveWithSetpointGenerator(
    ...
    return startRun(() -> previousTime.set(Timer.getFPGATimestamp()), () -> {
      ...
      SwerveSetpoint newSetpoint = setpointGenerator.generateSetpoint(
          prevSetpoint.get(), robotRelativeChassisSpeed.get(),
   ...
   * module states accordingly. Can use either open-loop or closed-loop velocity
   * control for the wheel velocities. Also has field- and robot-relative modes,
   ...
   *                      positive. Unaffected by field/robot relativity.
   * @param fieldRelative Drive mode. True for field-relative, false for
   *                      robot-relative.
   */
  public void drive(Translation2d translation, double rotation,
      boolean fieldRelative) {
    swerveDrive.drive(translation, rotation, fieldRelative, false); // Open loop
                                                                    // is
                                                                    // disabled
                                                                    // since it
                                                                    // shouldn't
  ...
  /**
   * Checks if the alliance is red, defaults to false if alliance isn't
   * available.
   *
   * @return true if the red alliance, false if blue. Defaults to false if none
   *         is available.
   */
  ...
  /**
   * This will zero (calibrate) the robot to assume the current position is
   * facing forward
   * <p>
  ...
  public ChassisSpeeds getTargetSpeeds(double xInput, double yInput,
    ...
    Translation2d scaledInputs = SwerveMath
        .cubeTranslation(new Translation2d(xInput, yInput));
  ...
  /**
   * Get the chassis speeds based on controller input of 1 joystick and one
   * angle. Control the robot at an offset of 90deg.
   *
   * @param xInput X joystick input for the robot to move in the X direction.
   * @param yInput Y joystick input for the robot to move in the Y direction.
